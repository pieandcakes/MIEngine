...
namespace MICore
{
    public class LocalTransport : PipeTransport
    {
        ...
        private static string NamedPipeName = "Microsoft-MIEngine-pipe-";
        ...
        public override void InitStreams(LaunchOptions options, out StreamReader reader, out StreamWriter writer)
        {
            ...
            string stdInName = String.Concat(NamedPipeName, Path.GetRandomFileName());
            string stdOutName = String.Concat(NamedPipeName, Path.GetRandomFileName());


            NamedPipeServerStream dbgStdInStream = new NamedPipeServerStream(stdInName);
            NamedPipeServerStream dbgStdOutStream = new NamedPipeServerStream(stdOutName);

            List<string> commandArgs = new List<String>();

            commandArgs.Add(localOptions.MIDebuggerPath);
            commandArgs.Add(localOptions.GetMIDebuggerArgs() + " --tty=CON");
            commandArgs.Add("2>1 \\\\.\\" + stdOutName);
            commandArgs.Add("< " + stdInName);

            HostOutputWindow.TryRunInTerminal(commandArgs, false, (pid) => { }, (message) => { });
            reader = new StreamReader(dbgStdOutStream);
            writer = new StreamWriter(dbgStdOutStream);
